...
For this analysis we queried data from the table at `world-fishing-827:scratch_cian.tdc_vessel_traffic_atba_2019_2021` which was created using the following [query](https://github.com/GlobalFishingWatch/pew_gfw_tristan_da_cunha/blob/main/queries/q_tdc_atba_vessel_traffic_voyages.sql). Note that this is a big table with more than 49 million rows. Therefore throughout this analysis we used queries to pull manageable subsets of these data.
...
```{r}
query_7 <- readr::read_file(str_c("queries", "q_tdc_atba_ports_visited_exclude_singapore.sql", sep="/")) 
```
