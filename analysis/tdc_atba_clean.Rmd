...
On 16 Mar 2011, the bulk carrier MS Oliva, en route from Brazil to Singapore, ran aground at Spinners Point on Nightingale Island. Soon afterwards, the vessel broke apart, spilling more than 300,000 gallons of oil and contaminating thousands of northern rockhopper penguins. The remoteness of Tristan da Cunha made mounting a rapid, large-scale containment logistically impossible. This event highlighted the elevated risk of environmental incidents due to the remoteness of Tristan da Cunha and its proximity to major shipping channels across the South Atlantic. To reduce the risk of another ship running aground, in April 2020 the government implemented a 20 nautical mile exclusion zone around each island within the archipelago, and designated these as Areas to be Avoided (ATBA) by transiting vessels greater than 400 gross tonnage (gt).

The Blue Belt programme is an initiative by the UK government to support the UK Overseas Territories with the protection and sustainable management of their marine environments. As part of this programme, the UK Marine Management Organisation (MMO) has been providing support to Tristan da Cunha by detecting transits through the ATBA and coordinating efforts to communicate the presence of the ATBA to relevant flag states and vessel operators.
...
2. Provide a full port-to-port track analysis of transiting vessels to identify which port authorities to prioritise in communicating the ATBA
...
For this analysis we queried data from the table location at `world-fishing-827:scratch_cian.tdc_vessel_traffic_atba_2019_2021`. This table was created using the following [query](https://github.com/GlobalFishingWatch/analysis-marine-manager-tristan-da-cunha/blob/main/queries/q-tdc-vessel-traffic-atba-w-voyages.sql). Note that this is a big table with more than 49 million rows. Therefore throughout this analysis we used queries to pull manageable subsets of data.

Load shapefiles for mapping:

```{r}
# Areas to be Avoided (ATBA)
atba_sf <- st_read("geodata/tdc_atba/ATBA_consolidate_25nm_buffer_wgs84.shp")
st_crs(atba_sf) <- 4326

# Shapefiles of Tristan da Cunha and Gough Island
# tdc_sf <- st_read("geodata/tristan-da-cunha-gough-island.shp")
tdc_sf <- st_read("geodata/tdc_osm/tristan_da_cunha_archipelago_osm.shp")
st_crs(tdc_sf) <- 4326

# Shapefile of Tristan EEZ only
...
```{r}
vt_gridded <- gfw_query(query = query_1,
                             run_query = TRUE,
                             con = con)$data
```
...
```{r}
saveRDS(vt_gridded, "data_production/data/gridded-vessel-activity-2019-2021.rds")
vt_gridded <- read_rds("data_production/data/gridded-vessel-activity-2019-2021.rds")
```
...
# set bounding area
bounding_1 <- transform_box(xlim = c(-18, -2), 
                            ylim = c(-48, -32),
...
m_grid <- add_little_globe(main_map = m_grid,
                           main_box = bounding_1,
                           globe_rel_size = 0.3,
                           globe_just = 'center',
...
Firstly, we analysed the total traffic, in terms of vessel hours, by vessels greater than 400 gross tonnes between 1 Jan. 2019 and 30 June 2021.

Query - vessel hours by > 400 gt vessels inside the **ATBA** between 01 Jan 2019 and 30 Jun 2021.
...
```{r}
vt_atba_summary <- gfw_query(query = query_2,
                             run_query = TRUE,
                             con = con)$data
```
...
```{r}
saveRDS(vt_atba_summary, "data_production/data/summary-vessel-traffic-atba-2019-2021.rds")
vt_atba_summary <- read_rds("data_production/data/summary-vessel-traffic-atba-2019-2021.rds")
```

Query - vessel hours by > 400 gt vessels inside the **EEZ** between 01 Jan 2019 and 30 Jun 2021.
...
```{r}
vt_eez_summary <- gfw_query(query = query_3,
                            run_query = TRUE,
                            con = con)$data
```
...
```{r}
saveRDS(vt_eez_summary, "data_production/data/summary-vessel-traffic-eez-2019-2021.rds")
...
ggplot() +
  ...
  annotate("text", x = ymd("2020-04-20"), y = 5000, colour = gfw_palette("chart")[5],
           label = "Implemntation of ATBA", angle = -90) +
...
To identify transits through the ATBA we first selected all of the locations by vessels over 400 gross tonnes within the ATBA. We then identified the `trip_id` associated with each of these positions, and selected every location associated with each `trip_id`.
...
2. Was stationary (speed < 0.2 knots) or stopped transmitting its location for more than 4 hours
3. Exited port (vessel more than 4km from port)

Define query - all positions from trips that passed within the ATBA:
...
```{r}
vt_atba_tracks <- gfw_query(query = query_4,
                            run_query = TRUE,
                            con = con)$data
```

Save the output locally and load from here.

```{r}
saveRDS(vt_atba_tracks, "data_production/data/vessl-tracks-over-400t-atba-only.rds")
vt_atba_tracks <- read_rds("data_production/data/vessl-tracks-over-400t-atba-only.rds")

# reformat the data so that inside_eez, inside_atba, and date are formatted correctly
vt_atba_tracks <- vt_atba_tracks %>% 
  mutate(inside_eez = inside_eez %>% as.logical(),
         inside_atba = inside_atba %>% as.logical(),
         date = lubridate::date(timestamp))
```

Because some vessels may have transited through the ATBA more than once, we need to create a `transit_id` to differentiate between individual vessel transits. To do this, we wrote a custom function called `create_transit_id()` to create a `transit_id`, and used `purrr::map()` to iterate through a list of unique vessel ids (`ssvid`). We applied this function to create two `transit_id`s; one for each transit through the EEZ and another for each transit through the ATBA. The steps of the function are:
...
create_transit_id <- function(ssvid, df, col_name = "inside_atba", output = "df", time_dif = 7){
  ...
  # return df or vector depending on specified output
  if(output == "df"){
    return(df_temp)
...
vt_transits <- purrr::map(.x = ssvid_list,
                         .f = create_transit_id,
                         df = vt_atba_tracks,
                         col_name = "inside_eez",
                         output = "df",
...
Save output to local device and load from here for future use:

```{r}
saveRDS(vt_transits, "data_production/data/vessel-traffic-eez-only-transit_id_fixed_v4.rds")
vt_transits <- read_rds("data_production/data/vessel-traffic-eez-only-transit_id_fixed_v4.rds")
```

For each `transit_id_eez` determine if the vessel:
...
Which vessels made the most transits? First we summarise the number of transits made through the ATBA, per vessel, per year. Including year allows us to merge these data with the identity information associated with that vessel id that year.
...
```{r}
vessels_by_n_transits <- gfw_query(query = query_5,
                                   run_query = TRUE,
                                   con = con)$data
```
...
```{r}
saveRDS(vessels_by_n_transits, "data_production/data/vessels_by_n_transits_v2.rds")
vessels_by_n_transits <- read_rds("data_production/data/vessels_by_n_transits_v2.rds")
```

After inspecting the data, there appear to be a number of sailing vessels that passed through the ATBA as part of the Vendee Globe round-the-world race. The algorithm used to predict gross tonnage of vessels generally performs well, with a predictive score of 0.77 (see Kroodsma et al. 2018), but the training data didn't include long-haul sailing vessels such as these, and subsequently confidence in the associated gross tonnage values is low. Additionally, the risk of these vessels running aground and causing a serious environmental incident, such as that caused by the MS Oliva, is low. Therefore we will exclude these vessels from further analysis:

First create a list of ssvids which most commonly self-reported as `Sailing`, and then filter out these ssvids from the transit data:
...
Looking at these vessels we can see that all of these vessels, wit hthe exception of the `LU HUANG YUAN YU 107` is flagged to FRA. The `LU HUANG YUAN YU 107` is definitely not a sailing vessel, so the self-reported value in this case is not accurate.

If we filter for only French-flagged vessels we get a more conservative list of sailing vessels, and so this is the exclusion criteria we will use:
...
So with that in mind, how many transits occurred before and after the implementation of the ATBA? Count the number of distinct `transit_id` values to count the number of transits.

```{r}
# 01 Jan 2019 - 30 Jun 2020
vt_transits %>% 
...
# limited to 12 months before/after
...
vt_transits %>% 
  ...
  ggplot() +
    ...
    labs(x = "Flag", y = "Transits through ATBA",
         ...
         subtitle = "1 April 2020 - 30 June 2021") +
    theme_gfw_cian() +
    theme(plot.title = element_text(size = 20),
          plot.subtitle = element_text(size = 16),
...
bounding_atba <- transform_box(xlim = c(-13.3, -9), 
                               ylim = c(-41, -36),
                               output_crs = "+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs")
...
vt_transits %>% 
  ...
  labs(title = "Transits by >400gt vessels through ATBA",
       subtitle = "1 April 2020 - 30 June 2021") +
  theme_gfw_map_cian() +
  theme(plot.title = element_text(size = 20),
        plot.subtitle = element_text(size = 16),
...
For additional context, let's plot the tracks of all squid jiggers that actually transited through the ATBA.

First, make a list of all squid jiggers that transited through the ATBA.
...
Rerun a modified version of the original query to select all of the port-to-port positions from the squid jigger vessels in `vt_transits`. 
...
```{r}
saveRDS(squid_tracks_2, "data_production/data/squid_jigger_tracks_full.rds")
squid_tracks_2 <- read_rds("data_production/data/squid_jigger_tracks_full.rds")
```
...
Use the `sf` (simple features) package to work out the distance to shore for every position within the ATBA. Ignore points outside the ATBA to save computing time. Create a `row_id` in `vt_transits` to merge results.

Create an `sf` object of points within the ATBA, and only include necessary columns:
...
Convert distance from metres to kilometres and round to 2 decimal places:
...
Update list of vessels to include min distance to shore

First calculate the min distance to shore for each transit:
```{r}
...
Save output for report and future analysis: 

```{r}
saveRDS(vessels_by_dist_no_sailing, "data_production/data/vessels_by_dist_no_sailing.rds")
write.csv(vessels_by_dist_no_sailing, "data_production/data/vessels_by_dist_no_sailing.csv", row.names = FALSE)
vessels_by_dist_no_sailing <- read_rds("data_production/data/vessels_by_dist_no_sailing.rds")
```

## Are vessels avoiding the ATBA?

What is the proportion of transits that pass through the EEZ that also pass through the ATBA? Does this proportion change during the 12 months before and after the implementation of the ATBA?
...
Using the same methodology, as described above, to identify likely port visits, we wanted to identify the port that each vessel was travelling from and to when it transited through the ATBA. To do this, used the `trip_id` associated with every trip that included a transit through the ATBA, to look up the name of the ports visited before and and after each transit.
...
```{r}
transit_ports <- gfw_query(query = query_7, 
                           run_query = TRUE,
                           con = con)$data
```
...
```{r}
saveRDS(transit_ports, "data_production/data/transit_ports_c4.rds")
transit_ports <- read_rds("data_production/data/transit_ports_c4.rds")
```

To make a map of origin and destination ports we need to reformat the `transit_ports` dataframe into a long version, with port label, lon, lat, and number of voyages starting and ending there.
...
# note there are 6 fewer destinations as 6 trips were still active at the end of our timerange
...
Create a map with points showing the locations of each port and the points sized in proportion to the number of transits beginning or ending there:

```{r, warning = FALSE, fig.width=10, fig.height=7}
# set the bounding are for the map
bounding_ports <- transform_box(xlim = c(min(ports_long$lon) - 1, 
...
Plot the number of transits that started or ended in the 10 ports most frequently visited by transiting vessels:
...
ports_long %>% 
  ...
  labs(x = "No. visits",
       y = "Port name",
       title = "Ten most visited ports by vessels\ntransiting through the ATBA") +
  theme_gfw_cian() +
...
Create a wide table of transits ending and beginning in each port to be included as an appendix in the final report.
...
ADD MORE DETAIL ON WHERE FILTERS INCLUDED IN QUERY - ASK TIM
...
```{r}
gap_events <- gfw_query(query = query_8,
                        run_query = TRUE,
                        con = con)$data
```
...
```{r}
saveRDS(gap_events, "data_production/data/ais_gaps_intentional.rds")
gap_events <- read_rds("data_production/data/ais_gaps_intentional.rds")
```

Reformat the data into a structure with a column for:
...
Which gaps cross the EEZ? Convert transits into spatial objects and use the spatial function `sf::st_intersects()` to return a TRUE/FALSE column for which transits intersected with the EEZ
...
# set bounding area
bounding_2 <- transform_box(xlim = c(-25, 5), 
                            ylim = c(-50, -25),
                            output_crs = "+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs")
...
```{r}
saveRDS(squid_tracks, "data_production/data/squid_jigger_tracks_gaps.rds")
...
```{r}
saveRDS(squid_gaps, "data_production/data/ais_gaps_squid.rds")
